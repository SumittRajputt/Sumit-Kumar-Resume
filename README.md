# Sumit-Kumar-Resume
**SUMIT KUMAR**
📍 New Delhi, India
📞 +91-8287281856 | 📧 [828kumarsumit@gmail.com](mailto:828kumarsumit@gmail.com)
🔗 LinkedIn: www.linkedin.com/in/sumit-kumar-649b45381 | 🔗 GitHub: github.com/yourprofile

---

### **PROFESSIONAL SUMMARY**

Aspiring Data Analyst with a Master’s in Data Science and hands-on experience in Python, SQL, Excel, and Power BI. Skilled in data cleaning, exploratory data analysis (EDA), and building dashboards to extract actionable insights. Strong analytical mindset with a focus on solving real-world business problems using data.

---

### **TECHNICAL SKILLS**

* **Programming:** Python (Pandas, NumPy)
* **Data Analysis:** SQL, Excel (Pivot Tables, VLOOKUP, Data Cleaning)
* **Visualization:** Power BI, Tableau (if applicable)
* **Tools:** Jupyter Notebook, Google Colab, Git & GitHub
* **Concepts:** Data Cleaning, EDA, KPI Analysis, Data Visualization

---

### **PROJECTS**

**Sales Performance Dashboard (Excel / Power BI)**

* Built an interactive dashboard to track KPIs including total sales, profit, and monthly growth trends
* Cleaned and transformed raw data for accurate reporting
* Identified top-performing products and regions, improving decision-making insights

**Customer Segmentation using Python**

* Performed data cleaning and EDA using Pandas and NumPy
* Segmented customers based on purchasing behavior
* Provided insights for targeted marketing strategies

**Retail Data Analysis using SQL**

* Wrote complex SQL queries including joins, aggregations, and filtering
* Analyzed customer purchasing patterns and sales trends
* Generated actionable insights to support business decisions

---

### **EDUCATION**

**Master of Science (M.S.) in Data Science**
(Your University Name) | Year

**Bachelor’s Degree**
(Your College Name) | Year

---

### **CERTIFICATIONS**

* Data Analytics Certification – (Coursera / Google / etc.)
* SQL / Excel / Power BI Certification

---

### **ADDITIONAL INFORMATION**

* Strong problem-solving and analytical skills
* Quick learner with interest in AI, ML, and data-driven decision-making
* Preparing for competitive exams alongside technical skill development

---
